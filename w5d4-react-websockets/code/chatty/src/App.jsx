...
const App = React.createClass({
  ...
  componentDidMount: function() {
    ...
    // this.socket = new WebSocket("ws://172.46.1.121:8080");
  ...
  addMessage: function(received_message) {
    ...
    // RIGHT HERE we need to figure what to do!
    switch (new_message.type) {
      case 'command':
        if (new_message.command == 'slap') {
          let username = new_message.username || 'Anonymous';
          let who = new_message.content.split(' ')[1];
          new_message.content = `${username} slaps ${who} with a big trout!`;
        }
      break;
    }
